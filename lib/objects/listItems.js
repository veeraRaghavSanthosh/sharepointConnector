    ...
    var doRequest = require('../util/doRequest')(client),
        ...
        crudl = {
            ...
            moveTo: function(oldFilePath, newFilePath, cb) {
                ...
                console.log("context url",client.BASE_LIST_URL.substring(0, client.BASE_LIST_URL.indexOf('_api/')) + '_api/contextinfo');
                doRequest({
                ...
                }, function(err, digestToken) {
                    if (err) {
                        return cb(err);
                    } else {
                        console.log("digestToken", digestToken.GetContextWebInformation);
                        ...
                        console.log("host", host);
                         doRequest({
                            ...
                            method: 'POST',
                            headers: {
                                'X-RequestDigest': digestToken.GetContextWebInformation ? digestToken.GetContextWebInformation.FormDigestValue : "",
                                'IF-MATCH': '*',
                                'content-Type': 'application/json;odata=verbose',
                                'X-HTTP-Method': 'MERGE'
                            }
